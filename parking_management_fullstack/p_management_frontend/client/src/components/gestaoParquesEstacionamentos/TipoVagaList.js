...
import TipoVagaForm from './TipoVagaForm';
...
class TipoVagaList extends React.Component {
    state = {
        lista: [],
        estado: 'carregando',
        msgErro: null,
    ...
    get_list = () => {
        ...
        fetch_server('/tipo_vaga', dadosIn, dadosOut => {
        ...
        fetch_server('/tipo_vaga', dadosIn, dadosOut => {
            if (dadosOut.err) this.setState({ estado: 'erro', msgErro: dadosOut.err });
                ...
                this.setState({
                    ...
                    estado: 'carregado',
                    itemEditar: {
                        id_tipo_vaga: dadosOut.dados.max_id + 1,
    ...
    delete_item = id => {
        ...
        const dadosIn = { opcao: 'delete', dados: { id_tipo_vaga: id } };
                                    ...
                                    {lista.map((item, index) => (
                                        <TableRow key={index}>
                                            <TableCell component='th' scope='row'>
                                                ...
                                                <IconButton
                                                    onClick={() => {
                                                        this.delete_item(item.id_tipo_vaga);
                                                    }}
                                                >
                                                    <DeleteIcon />
                                                </IconButton>
                                            ...
                                            <TableCell>{item.descricao_tipo_vaga}</TableCell>
                                            <TableCell>{item.dh_inclusao_tipo_vaga}</TableCell>
                                            <TableCell>{item.dh_alteracao_tipo_vaga}</TableCell>
                                            <TableCell>{item.st_tipo_vaga}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
...
TipoVagaList.propTypes = {
    classes: PropTypes.object.isRequired,
    loginData: PropTypes.object
};

export default withStyles(styles)(TipoVagaList);
