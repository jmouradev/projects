...
import MarcaVeiculoForm from './MarcaVeiculoForm';
...
class MarcaVeiculoList extends React.Component {
    state = {
        lista: [],
        estado: 'carregando',
        msgErro: null,
        ...
        fetch_server('/marca_veiculo', dadosIn, dadosOut => {
            if (dadosOut.err) this.setState({ estado: 'erro', msgErro: dadosOut.err });
                ...
                this.setState({
                    ...
                    estado: 'carregado',
                    itemEditar: {
                        id_marca_veiculo: dadosOut.dados.max_id + 1,
                        dh_inclusao_marca_veiculo: date,
                        dh_alteracao_marca_veiculo: date,
                        st_marca_veiculo: 'A'
                    },
                    opcao: 'insert'
                });
    ...
    delete_item = id => {
        if (!window.confirm('Eliminar Marca de Veículo?')) return;

        const dadosIn = { opcao: 'delete', dados: { id_marca_veiculo: id } };
                                    ...
                                    {lista.map((item, index) => (
                                        <TableRow key={index}>
                                            <TableCell component='th' scope='row'>
                                                ...
                                                <IconButton
                                                    onClick={() => {
                                                        this.delete_item(item.id_marca_veiculo);
                                                    }}
                                                >
                                                    <DeleteIcon />
                                                </IconButton>
                                            </TableCell>
                                            <TableCell>{item.id_marca_veiculo}</TableCell>
                                            <TableCell>{item.descricao_marca_veiculo}</TableCell>
                                            <TableCell>{item.dh_inclusao_marca_veiculo}</TableCell>
                                            <TableCell>{item.dh_alteracao_marca_veiculo}</TableCell>
                                            <TableCell>{item.st_marca_veiculo}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
...
MarcaVeiculoList.propTypes = {
    classes: PropTypes.object.isRequired,
    loginData: PropTypes.object
};

export default withStyles(styles)(MarcaVeiculoList);
