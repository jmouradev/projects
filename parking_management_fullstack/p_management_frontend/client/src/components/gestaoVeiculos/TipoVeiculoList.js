...
class TipoVeiculoList extends React.Component {
    state = {
        lista: [],
        estado: 'carregando',
        msgErro: null,
        ...
        fetch_server('/tipo_veiculo', dadosIn, dadosOut => {
            if (dadosOut.err) this.setState({ estado: 'erro', msgErro: dadosOut.err });
                ...
                this.setState({
                    ...
                    estado: 'carregado',
                    itemEditar: {
                        id_tipo_veiculo: dadosOut.dados.max_id + 1,
                        dh_inclusao_tipo_veiculo: date,
                        dh_alteracao_tipo_veiculo: date,
                        st_tipo_veiculo: 'A'
                    },
                    opcao: 'insert'
                });
    ...
    delete_item = id => {
        if (!window.confirm('Eliminar Tipo de Veículo?')) return;

        const dadosIn = { opcao: 'delete', dados: { id_tipo_veiculo: id } };
                                    ...
                                    {lista.map((item, index) => (
                                        <TableRow key={index}>
                                            <TableCell component='th' scope='row'>
                                                ...
                                                <IconButton
                                                    onClick={() => {
                                                        this.delete_item(item.id_tipo_veiculo);
                                                    }}
                                                >
                                                    <DeleteIcon />
                                                </IconButton>
                                            </TableCell>
                                            <TableCell>{item.id_tipo_veiculo}</TableCell>
                                            <TableCell>{item.descricao_tipo_veiculo}</TableCell>
                                            <TableCell>{item.dh_inclusao_tipo_veiculo}</TableCell>
                                            <TableCell>{item.dh_alteracao_tipo_veiculo}</TableCell>
                                            <TableCell>{item.st_tipo_veiculo}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
...
TipoVeiculoList.propTypes = {
    classes: PropTypes.object.isRequired,
    loginData: PropTypes.object
};

export default withStyles(styles)(TipoVeiculoList);
