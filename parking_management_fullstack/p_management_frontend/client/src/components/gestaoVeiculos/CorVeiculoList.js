...
import CorVeiculoForm from './CorVeiculoForm';
...
class CorVeiculoList extends React.Component {
    state = {
        lista: [],
        estado: 'carregando',
        msgErro: null,
        ...
        fetch_server('/cor_veiculo', dadosIn, dadosOut => {
            if (dadosOut.err) this.setState({ estado: 'erro', msgErro: dadosOut.err });
                ...
                this.setState({
                    ...
                    estado: 'carregado',
                    itemEditar: {
                        id_cor_veiculo: dadosOut.dados.max_id + 1,
                        dh_inclusao_cor_veiculo: date,
                        dh_alteracao_cor_veiculo: date,
                        st_cor_veiculo: 'A'
                    },
                    opcao: 'insert'
                });
    ...
    delete_item = id => {
        if (!window.confirm('Eliminar Cor de Veículo?')) return;

        const dadosIn = { opcao: 'delete', dados: { id_cor_veiculo: id } };
                                    ...
                                    {lista.map((item, index) => (
                                        <TableRow key={index}>
                                            <TableCell component='th' scope='row'>
                                                ...
                                                <IconButton
                                                    onClick={() => {
                                                        this.delete_item(item.id_cor_veiculo);
                                                    }}
                                                >
                                                    <DeleteIcon />
                                                </IconButton>
                                            </TableCell>
                                            <TableCell>{item.id_cor_veiculo}</TableCell>
                                            <TableCell>{item.descricao_cor_veiculo}</TableCell>
                                            <TableCell>{item.dh_inclusao_cor_veiculo}</TableCell>
                                            <TableCell>{item.dh_alteracao_cor_veiculo}</TableCell>
                                            <TableCell>{item.st_cor_veiculo}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
...
CorVeiculoList.propTypes = {
    classes: PropTypes.object.isRequired,
    loginData: PropTypes.object
};

export default withStyles(styles)(CorVeiculoList);
