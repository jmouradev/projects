...
import NivelAcessoForm from './NivelAcessoForm';
...
class NivelAcessoList extends React.Component {
    state = {
        lista: [],
        estado: 'carregando',
        msgErro: null,
        ...
        fetch_server('/nivel_acesso', dadosIn, dadosOut => {
            if (dadosOut.err) this.setState({ estado: 'erro', msgErro: dadosOut.err });
                ...
                this.setState({
                    ...
                    estado: 'carregado',
                    itemEditar: {
                        id_nivel_acesso: dadosOut.dados.max_id + 1,
                        dh_inclusao_nivel_acesso: date,
                        dh_alteracao_nivel_acesso: date,
                        st_nivel_acesso: 'A'
                    },
                    opcao: 'insert'
                });
    ...
    delete_item = id => {
        if (!window.confirm('Eliminar Nivel de Acesso?')) return;

        const dadosIn = { opcao: 'delete', dados: { id_nivel_acesso: id } };
                                    ...
                                    {lista.map((item, index) => (
                                        <TableRow key={index}>
                                            <TableCell component='th' scope='row'>
                                                ...
                                                <IconButton
                                                    onClick={() => {
                                                        this.delete_item(item.id_nivel_acesso);
                                                    }}
                                                >
                                                    <DeleteIcon />
                                                </IconButton>
                                            </TableCell>
                                            <TableCell>{item.id_nivel_acesso}</TableCell>
                                            <TableCell>{item.descricao_nivel_acesso}</TableCell>
                                            <TableCell>{item.dh_inclusao_nivel_acesso}</TableCell>
                                            <TableCell>{item.dh_alteracao_nivel_acesso}</TableCell>
                                            <TableCell>{item.st_nivel_acesso}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
...
NivelAcessoList.propTypes = {
    classes: PropTypes.object.isRequired,
    loginData: PropTypes.object
};

export default withStyles(styles)(NivelAcessoList);
